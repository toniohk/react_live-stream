...
const VerticalLayout = props => {
  ...
  const singularStore = useSelector(state => state.singular)
  const monthlyStore = useSelector(state => state.monthly)
  const loading = singularStore.loading || monthlyStore.loading
      ...
      >
        <div className='navbar-container d-flex content'>
          {navbar ? (
            ...
            <NavbarComponent setMenuVisibility={setMenuVisibility} skin={skin} setSkin={setSkin} routerProps={routerProps} />
          )}
        </div>
      </Navbar>
      {children}
